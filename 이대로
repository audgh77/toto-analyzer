...
# 팀 선택
st.subheader("🏟️ 팀 선택")
teams = ["Man City", "Liverpool", "Arsenal", "Chelsea", "Tottenham", "Newcastle", "Brighton"]
home_team = st.selectbox("홈팀", teams, index=0)
away_team = st.selectbox("원정팀", teams, index=1)
...
col3.metric("경기", f"{home_team} vs {away_team}")
...
# 최근 폼
st.subheader("⚽ 최근 폼")
st.text(f"{home_team}: 승승패무승")
st.text(f"{away_team}: 승무승승패")
